  ...
  {:else}
    <div class="divide-y divide-white/5">
      {#each entries as entry, i (entry.id)}
        <div class="flex items-center px-6 py-4 hover:bg-white/5 transition-colors {i < 3 ? 'bg-white/[0.02]' : ''}">
          ...
          <div class="flex-1 ml-4">
            <div class="font-semibold text-white {i === 0 ? 'text-amber-300' : ''}">{entry.playerName}</div>
            <div class="text-xs text-slate-500">{formatDate(entry.createdAt)}</div>
          </div>
